...
- 対象: SSD正本ルールを読んだ専用クリーンクローンにある `manapick-career` リポジトリのニュース、出典レジストリ、日本語OG画像
- 実行データ: `/Volumes/SSD256GB/My Folder/02_CODEX/Manapick/03_AUTOMATION/.cache/manapick-career-news/` 配下だけを使う。`/private/tmp`、旧Documents、内蔵ディスクへclone・ログ・キャッシュを作らない
- 状態: ACTIVE
...
2026-07-23の08:00枠では、当時のfreshな `/private/tmp` cloneから候補作成と専用ブランチへのcommitまで到達した。ブランチは `automation/career-news-20260723-080053`、commitは `b7efe3360a78d6da2cc8d7e7a9f83e29d14328b0` である。ただしGitHubでのDraft PR作成はHTTP 403で失敗したため、PR URLはなく、PR作成成功とは扱わない。commitは専用ブランチにだけ存在し、`main` は変更されず、記事も公開されていない。この`/private/tmp`運用は現行のSSD保管ルールに適合しないため廃止した。
...
2026-08-02にSSD正本移行後の運用を再確認した。実行開始時に`/Volumes/SSD256GB`のmountを検査し、rootの`AGENTS.md`と`README_STORAGE_RULES.md`を読む。clone、ログ、候補、npm cacheなどの一時データは`03_AUTOMATION/.cache/manapick-career-news`配下の実行ごとのディレクトリへ限定し、SSDが未mountなら変更せず終了する。登録値の反映後に初回成功実行を確認するまでは、SSD上の実行実績を「未確認」とする。

...
export function CareerExplorer({jobs,categories}:{jobs:CareerJob[];categories:Category[]}){
  ...
  return <section className="explorer" aria-labelledby="explorer-title"><div className="explorer-head"><div><span className="eyebrow">人が確認した{jobs.length}職業</span><h2 id="explorer-title">仕事内容まで読める職業を探す。</h2><p className="explorer-directory-link"><a href="/all/">556職業の公式名録から探す →</a></p></div><label className="search-box"><Search aria-hidden="true"/><span className="sr-only">職業を検索</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="例：数字、デザイン、人を支える"/></label></div>

...
return <div className="compare-tool"><div className="compare-picker"><strong>比較する職業（最大3つ）</strong><div>{jobs.map(job=><button key={job.slug} className={selected.includes(job.slug)?"selected":""} onClick={()=>toggle(job.slug)} aria-pressed={selected.includes(job.slug)}>{job.name}{selected.includes(job.slug)?<X aria-hidden="true"/>:null}</button>)}</div></div><div className="compare-scroll" tabIndex={0} aria-label="職業比較表"><table><thead><tr><th>比較項目</th>{chosen.map(job=><th key={job.slug}>{job.name}</th>)}</tr></thead><tbody>
  ...
  <tr><th>業界で担う役割</th>{chosen.map(job=><td key={job.slug}>{careerInsightBySlug.get(job.slug)?.industryRole}</td>)}</tr>
  <tr><th>似た職業との違い</th>{chosen.map(job=><td key={job.slug}><ul>{careerInsightBySlug.get(job.slug)?.adjacentJobs.map(item=>{const adjacent=jobs.find(candidate=>candidate.slug===item.slug);return <li key={item.slug}><strong>{adjacent?.name}</strong><small>{item.difference}</small></li>})}</ul></td>)}</tr>
  ...
  <tr><th>転職準備を左右する条件</th>{chosen.map(job=><td key={job.slug}><ul>{careerInsightBySlug.get(job.slug)?.transitionFactors.map(item=><li key={item}>{item}</li>)}</ul></td>)}</tr>
  <tr><th>応募前に作る証拠</th>{chosen.map(job=><td key={job.slug}><ul>{careerInsightBySlug.get(job.slug)?.evidenceToPrepare.map(item=><li key={item}>{item}</li>)}</ul></td>)}</tr>
  ...
  <tr><th>社内での活かし方</th>{chosen.map(job=><td key={job.slug}><ul>{careerInsightBySlug.get(job.slug)?.internalUse.map(item=><li key={item}>{item}</li>)}</ul></td>)}</tr>

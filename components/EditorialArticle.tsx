...
import type { EditorialSection } from "@/content/editorial";
...
export function EditorialArticle({
  ...
  return <>
    ...
    <div className="article-layout">
      <aside className="article-toc" aria-label="この記事の目次"><p><ListTree aria-hidden="true" />目次</p>{toc}<small><Clock3 aria-hidden="true" />読了目安 {readMinutes}分</small></aside>
      <article className="editorial-article">
        ...
        {sections.map((section, index) => <section id={section.id} key={section.id} className="article-section"><span className="article-section-number">SECTION {String(index + 1).padStart(2, "0")}</span><h2>{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.points && <ul>{section.points.map((point) => <li key={point}><CheckCircle2 aria-hidden="true" />{point}</li>)}</ul>}</section>)}

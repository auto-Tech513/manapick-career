...
export default function ResearchPage() {
  ...
  const graph = { "@context": "https://schema.org", "@graph": [
    ...
    { "@type": "ItemList", itemListElement: picked.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.title, url: absoluteUrl(`/news/${item.slug}/`) })) },
  ] };
  return <><JsonLd data={graph} /><div className="page-shell research-page">
    <header className="page-heading research-hero"><span className="eyebrow">career evidence room</span><h1>数字を、焦らせる材料にしない。</h1><p>雇用、賃金、学び、AI・DXの一次資料を、意味・限界・次の確認へ分けます。市場の数字を職業順位やあなたの成功確率には変換しません。</p><div className="research-stats"><span><strong>{newsItems.length}</strong>出典付き解説</span><span><strong>{sites.length}</strong>参照サイト比較</span><span><strong>{deepCount}</strong>主要画面まで確認</span></div></header>
    ...
    <section className="research-latest" aria-labelledby="research-latest-title"><div className="section-heading"><span className="eyebrow">primary-source briefings</span><h2 id="research-latest-title">一次資料から読む6つの入口</h2><p>公表日の新しさではなく、いま確認したい質問から選べます。</p></div><div className="research-card-grid">{picked.map((item) => <Link href={`/news/${item.slug}/`} key={item.slug}><small>{item.kind}・確認 {item.checkedAt}</small><h3>{item.title}</h3><p>{item.answer}</p><span>意味と限界を読む <ArrowRight aria-hidden="true" /></span></Link>)}</div><Link className="text-link" href="/news/">30件のキャリアニュースを見る <ArrowRight aria-hidden="true" /></Link></section>

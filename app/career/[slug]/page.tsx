...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const job=publishedJobs.find(x=>x.slug===slug);if(!job)return{};const title=`${job.name}になるには？仕事内容・必要スキル・学ぶ順番`;const description=`${job.conclusion.join(" ")} 資格・AI・無料学習動画まで公式情報から整理。`;const image=articleOgUrl("career",job.slug,`${job.name}|${job.checkedAt}`);return{title,description,alternates:{canonical:`/career/${job.slug}/`},openGraph:{title,description,url:absoluteUrl(`/career/${job.slug}/`),type:"article",siteName:"manapick career",locale:"ja_JP",modifiedTime:job.checkedAt,images:[{url:image,width:1200,height:630,alt:`${job.name}の仕事内容と学びの入口`}]},twitter:{card:"summary_large_image",title,description,images:[image]}}}
...
export default async function CareerPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const job=publishedJobs.find(x=>x.slug===slug);if(!job)notFound();const category=categories.find(x=>x.key===job.category);const sources=sourceRegistry.filter(source=>job.sourceIds.includes(source.sourceId));const url=absoluteUrl(`/career/${job.slug}/`);const graph={"@context":"https://schema.org","@graph":[
  {"@type":"Article","@id":`${url}#article`,headline:`${job.name}になるには`,description:job.conclusion.join(" "),url,image:articleOgUrl("career",job.slug,`${job.name}|${job.checkedAt}`),dateModified:job.checkedAt,inLanguage:"ja-JP",author:{"@type":"Organization",name:job.author,url:absoluteUrl("/operator/")},editor:{"@type":"Organization",name:job.editor,url:absoluteUrl("/operator/")},publisher:{"@id":absoluteUrl("/#organization")},mainEntityOfPage:url,citation:sources.map(x=>x.url)},
  ...
  <div className="detail-layout"><article className="detail-content">
    <section id="work"><span className="section-num">01</span><h2>どんな仕事？</h2>{job.work.map(x=><p key={x}>{x}</p>)}<ul className="task-list">{job.tasks.map(x=><li key={x}><CheckCircle2 aria-hidden="true"/>{x}</li>)}</ul></section>
    <section id="considerations"><span className="section-num">02</span><h2>向いている可能性がある人</h2><p className="section-lead">適性の断定ではなく、仕事を調べるときの判断材料です。</p><ul className="check-list">{job.possibilities.map(x=><li key={x}>{x}</li>)}</ul></section>
    <section id="cautions"><span className="section-num">03</span><h2>先に知っておきたい注意点</h2><div className="caution-box"><CircleAlert aria-hidden="true"/><div>{job.cautions.map(x=><p key={x}>{x}</p>)}</div></div></section>
    <section id="learning"><span className="section-num">04</span><h2>必要スキルと学ぶ順番</h2><div className="chips large">{job.skills.map(x=><span key={x}>{x}</span>)}</div><ol className="learning-steps">{job.learningSteps.map((x,i)=><li key={x}><span>{String(i+1).padStart(2,"0")}</span><p>{x}</p></li>)}</ol><div className="time-note"><strong>学習期間の考え方</strong><p>{job.learningTime}</p></div></section>
    <section id="next-steps"><span className="section-num">05</span><h2>学び・AI・資格へ進む</h2><p className="section-lead">各サイトの役割を分け、確認済みの個別ページだけへつなぎます。</p><div className="resource-groups">
    ...
    <section className="editorial-block" id="sources"><span className="section-num">06</span><h2>この記事の確認方法</h2><dl><div><dt>執筆</dt><dd>{job.author}</dd></div><div><dt>編集確認</dt><dd>{job.editor}</dd></div><div><dt>最終確認日</dt><dd>{job.checkedAt}</dd></div><div><dt>作成方法</dt><dd>{job.editorNote}</dd></div></dl><h3>出典</h3><ul className="source-list">{sources.map(source=><li key={source.sourceId}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.provider}<ArrowUpRight aria-hidden="true"/></a><small>確認日 {source.checkedAt}／{source.notes}</small></li>)}</ul><p>AI補助を利用した場合も、公開前に編集者が全主張とリンクを確認します。誤りにお気づきの場合は<Link href="/contact/">訂正窓口</Link>をご利用ください。</p></section>
  </article><aside className="detail-aside"><CareerToc/><Link className="button secondary full" href="/compare/">ほかの仕事と比較する</Link></aside></div>
</div></>}

function CareerToc(){return <nav aria-label="このページの目次"><strong>このページで分かること</strong><a href="#summary">先に結論</a><a href="#work">仕事内容</a><a href="#considerations">判断材料</a><a href="#cautions">注意点</a><a href="#learning">スキルと学ぶ順番</a><a href="#next-steps">動画・AI・資格</a><a href="#sources">出典と確認方法</a></nav>}
